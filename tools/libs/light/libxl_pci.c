...
static void pcid_parameters_common_add(libxl__gc *gc, libxl__json_object **param,
...
static void libxl__pcid_param_add_string(libxl__gc *gc, libxl__json_object **param,
                                         const char *name, const char *s)
{
    ...
    pcid_parameters_common_add(gc, param, name, obj);
}
...
static libxl_domid vchan_find_pcid_server(libxl_ctx *ctx)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (sscanf(domains[i], "%d", &d) != 1)
            continue;
        if (xs_path_exists(gc, GCSPRINTF(PCID_XS_PATH, d)) > 0) {
...
static int vchan_init_client(libxl__gc *gc, struct vchan_state *state)
{
    ...
    state->xs_path = GCSPRINTF(PCID_XS_PATH, state->domid);
                               ...
                               int id)
{
    ...
    libxl__yajl_gen_asciiz(hand, PCID_MSG_EXECUTE);
                              ...
                              libxl__json_object **o_r)
{
    size_t len;
    char *end = NULL;
    const size_t eoml = sizeof(PCID_END_OF_MESSAGE) - 1;
    ...
    end = memmem(state->rx_buf, state->rx_buf_used, PCID_END_OF_MESSAGE, eoml);
    if (!end)
        return ERROR_NOTFOUND;
                                ...
                                const libxl__json_object **result)
{
    ...
    *result = NULL;
    ...
    case LIBXL__PCID_MESSAGE_TYPE_RETURN:
        *result = libxl__json_map_get(PCID_MSG_RETURN, resp, JSON_ARRAY);
        if (!(*result))
            *result = libxl__json_map_get(PCID_MSG_RETURN, resp, JSON_STRING);
        return 0;
                            ...
                            const libxl__json_object **result)
{
    ...
    while (true) {
        ...
        if (state->rx_buf_size - state->rx_buf_used < PCI_RECEIVE_BUFFER_SIZE) {
            size_t newsize = state->rx_buf_size * 2 + PCI_RECEIVE_BUFFER_SIZE;

            if (newsize > PCI_MAX_SIZE_RX_BUF) {
                LOGD(ERROR, state->domid,
                     "pcid receive buffer is too big (%zu > %lld)",
                     newsize, PCI_MAX_SIZE_RX_BUF);
                           ...
                           libxl_device_pci *pci)
{
    ...
    if (strcmp(SYSFS_PCI_DEV, sysfs_path) == 0)
        libxl__pcid_param_add_string(gc, &args, PCID_CMD_DIR_ID, PCID_PCI_DEV);
    else if (strcmp(SYSFS_PCIBACK_DRIVER, sysfs_path) == 0)
        libxl__pcid_param_add_string(gc, &args, PCID_CMD_DIR_ID, PCID_PCIBACK_DRIVER);
    else if (strcmp(SYSFS_DRIVER_PATH, sysfs_path) == 0)
        libxl__pcid_param_add_string(gc, &args, PCID_CMD_DIR_ID, SYSFS_DRIVER_PATH);

    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_PATH, pci_path);
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO, buf);
...
libxl_device_pci *libxl_device_pci_assignable_list(libxl_ctx *ctx, int *num)
{
    ...
    *num = 0;
    ...
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_DIR_ID,
                                 PCID_PCIBACK_DRIVER);
                            ...
                            char **driver_path)
{
    ...
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_PATH, pci_path);
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO, pci_info);
...
static uint16_t sysfs_dev_get_vendor(libxl__gc *gc, libxl_device_pci *pci)
{
    ...
    if (!vchan)
        goto fail;
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO,
...
static int pci_multifunction_check(libxl__gc *gc, libxl_device_pci *pci, unsigned int *func_mask)
{
    *func_mask = 0;
    ...
    for (i = 0; (dir = libxl__json_array_get(result, i)); i++) {
        ...
        libxl__pcid_param_add_string(gc, &args, PCID_CMD_DIR_ID, PCID_PCIBACK_DRIVER);
        libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO, path);
                            ...
                            int rc)
{
    ...
    resource_path = GCSPRINTF("/"PCI_BDF"/resource", pci->domain,
                              pci->bus, pci->dev, pci->func);
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO, resource_path);
    ...
    irq_path = GCSPRINTF("/"PCI_BDF"/irq", pci->domain,
                         pci->bus, pci->dev, pci->func);
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO, irq_path);
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_DIR_ID, PCID_PCI_DEV);
    result = vchan_send_command(gc, vchan, PCID_CMD_READ_HEX, args);
    if (!result) {
                                   ...
                                   unsigned int dev, unsigned int func)
{
    ...
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_PATH, reset);
    libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO, buf);
    result = vchan_send_command(gc, vchan, PCID_CMD_WRITE, args);
    if (!result) {
...
static void do_pci_remove(libxl__egc *egc, pci_remove_state *prs)
{
    ...
    if (type == LIBXL_DOMAIN_TYPE_HVM) {
        ...
        if (!vchan)
            goto out_fail;
        libxl__pcid_param_add_string(gc, &args, PCID_CMD_PCI_INFO, sysfs_path);
        result = vchan_send_command(gc, vchan, PCID_CMD_READ_RESOURCES, args);
        if (!result) {
